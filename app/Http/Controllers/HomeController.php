    ...
    + IF(DATE_FORMAT(CURRENT_DATE(), '%m-%d') > DATE_FORMAT(doj, '%m-%d'), 1, 0) YEAR), CURRENT_DATE()) " . $order);
        ...
        $formattedResults = $paginated->map(function ($item) use ($currentDate, $currentYear) {
            ...
            $daysLeft = $currentDate->diffInDays($anniversaryDate);
                ...
                if ($daysLeft === 0) {
                    $emoji = ' 🥳';
                    $label = '<span class="badge bg-primary mt-2">' . $yearDifference . '<sup>' . $ordinalSuffix . '</sup> ' . get_label('work_anniversary', 'Work Anniversary') . ' ' . get_label('today', 'Today') . '</span>' . $emoji;
                } elseif ($daysLeft === 1) {
            ...
            return [
                ...
                'type' => $item['type'] === 'user' ? '<span class="badge bg-primary">' . get_label('user', 'User') . '</span>' : '<span class="badge bg-info">' . get_label('client', 'Client') . '</span>',
            ];
        });
