@extends('layout')
@section('title')
<?= $is_favorite == 1 ? get_label('favorite_projects', 'Favorite projects') : get_label('projects', 'Projects') ?> - <?= get_label('kanban_view', 'Kanban View') ?>
@endsection
@php
...
<div class="container-fluid">
    ...
    @endphp
    <x-projects-kanban-card :projects="$projects" :statuses="$statuses" :showSettings="$showSettings" :canEditProjects="$canEditProjects" :canDeleteProjects="$canDeleteProjects" :canDuplicateProjects="$canDuplicateProjects" :webGuard="$webGuard" />
    @else
    <?php $type = 'projects'; ?>
